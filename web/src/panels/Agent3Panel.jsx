...
import Agent3Analytics from './analytics/Agent3Analytics'

const ACCENT = '#4A9EE8'

export default function Agent3Panel() {
  ...
  const { hasMessages } = useChat('agent3')
  ...
  const handleAskAgent = (name) => {
    setArtistFocus(name)
    setInitialMessage(`Summarize the audience profile for ${name}`)
    setMobileView('chat')
  }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          <ChatPanel
            agentKey="agent3"
            agentName="Audience-Fit"
            agentSub="Who to Play With"
            accentColor={ACCENT}
            artistOptions={artistOptions}
            suggestedQuestions={AGENT_SUGGESTIONS.agent3}
