...
import Agent2Analytics from './analytics/Agent2Analytics'

const ACCENT = '#8B7FE8'

export default function Agent2Panel() {
  ...
  const { hasMessages } = useChat('agent2')

  const artistOptions = data?.map(a => a.artist_name) ?? []

  // Carry-over artist from previous agent
  useEffect(() => {
  ...
  const handleAskAgent = (name) => {
    setArtistFocus(name)
    setInitialMessage(`Tell me about the brand strategy for ${name}`)
    setMobileView('chat')
  }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          <ChatPanel
            agentKey="agent2"
            agentName="Strategy Synthesis"
            agentSub="How to Play"
            accentColor={ACCENT}
            artistOptions={artistOptions}
            suggestedQuestions={AGENT_SUGGESTIONS.agent2}
